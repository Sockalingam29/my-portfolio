            ...
            <div className="timeline-panel">
                <div >
                    ...
                    <h5 className='mt-1 text-sm text-neutral-400'>
                        {new Intl.DateTimeFormat('en-US', { month: 'short' }).format(new Date(item.startDate)) + " " + new Date(item.startDate).getFullYear()} - {item.endDate !== 'Present' ? new Intl.DateTimeFormat('en-US', { month: 'short' }).format(new Date(item.endDate)) + " " + new Date(item.endDate).getFullYear() : "Present"} · {Math.round((new Date(item.endDate == "Present" ? Date.now() : item.endDate) - new Date(item.startDate)) / (1000 * 60 * 60 * 24 * 30))} mos
                    </h5>
